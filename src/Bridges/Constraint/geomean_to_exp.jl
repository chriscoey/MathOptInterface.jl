...
function bridge_constraint(::Type{GeoMeantoExpBridge{T, F, G, H, I}}, model::MOI.ModelLike, f::H, s::MOI.GeometricMeanCone) where {T, F, G, H, I}
    ...
    le_y_index = MOI.add_constraint(model, MOIU.operate(-, T, f_scalars[1], MOI.SingleVariable(y)), MOI.LessThan(zero(T)))
    ge_z_index = MOI.add_constraint(model, MOIU.operate(sum, T, z), MOI.GreaterThan(zero(T)))
